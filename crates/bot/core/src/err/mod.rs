...
pub async fn bassness_error(ctx: &mut Context, err: &SfError) -> Result<Option<String>> {
    match err {
        SfError::Eyre(_) => Ok(None),
        SfError::MongoError(_) => Ok(None),
        SfError::TrainingsError(err) => training_error(ctx, err).await,
        SfError::CalendarError(err) => todo!(),
        SfError::UserError(err) => todo!(),
        SfError::RequestError(err) => todo!(),
    }
}
